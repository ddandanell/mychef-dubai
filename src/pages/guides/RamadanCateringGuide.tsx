...
const faqs = [
  ...
  },
  {
    ...
    a: 'How to plan iftar catering Dubai starts from a set format that we adjust to your event rather than selling a fixed box: menu length, service style, staff and equipment are chosen for the day. Ask for the format closest to what you are planning and we shape it from there.',
  },
]
          ...
          <div className="ram-menus grid md:grid-cols-3 gap-6">
            {menus.map((menu, i) => (
              <div key={i} className="ram-menu-card bg-white p-8 opacity-0 translate-y-10">
                ...
                <p className="font-inter text-body-sm text-gray-500 leading-relaxed">{menu.description}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="ram-gatherings grid md:grid-cols-3 gap-6">
            {largeGatherings.map((g, i) => (
              <div key={i} className="ram-gather-card bg-black p-8 opacity-0 translate-y-12">
                ...
                <p className="font-inter text-body-sm text-gray-400 leading-relaxed">{g.description}</p>
              </div>
            ))}
          </div>
        </div>

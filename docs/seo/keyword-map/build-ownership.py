...
<td class="nums">{('#' + str(r['current_position'])) if r['current_position'] else '—'} → {r['target_position']}</td><td><code>{esc(r['primary_owning_url'])}</code>{('<div class=muted>' + ' '.join('<code>' + esc(u) + '</code>' for u in r['secondary_supporting_urls'][:3]) + '</div>') if r['secondary_supporting_urls'] else ''}</td>
...
<main><table><thead><tr><th>Keyword</th><th>Volume · intent</th><th>Value /mo</th><th>KD</th><th>Position → target</th><th>Owner · supporting</th><th>T M H1 H2 ×body FAQ · anchor</th><th>Cannibalisation</th><th>Competitor gap</th><th>Score</th><th>Next action</th></tr></thead><tbody>{trs}</tbody></table>

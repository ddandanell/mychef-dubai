...
cur.execute(DDL)
for tbl, col, typ in (("seo_keywords", "page_visitors", "INT"), ("seo_keywords", "page_pageviews", "INT"),
...
if kw:
    psycopg2.extras.execute_values(cur, """INSERT INTO seo_keywords (run_id, keyword, owner_url, role, search_volume, intent, commercial_value, difficulty, current_position, target_position, secondary_urls,
        title_cov, meta_cov, h1_cov, h2_cov, body_count, faq_cov, anchor_cov, cannibalisation, competitor_gap, serp_similarity, score, next_action, silo,
        page_visitors, page_pageviews) VALUES %s ON CONFLICT DO NOTHING""",
        [(run_id, r["keyword"] or "(untargeted)", r["primary_owning_url"], r["role"], r["search_volume"], r["intent"], r["commercial_value"], r["difficulty"], r["current_position"], r["target_position"], r["secondary_supporting_urls"],
          r["title_coverage"], r["meta_coverage"], r["h1_coverage"], r["h2_coverage"], r["body_coverage"], r["faq_coverage"], r["internal_anchor_coverage"], r["cannibalisation_risk"], r["competitor_gap"], r["serp_similarity"], r["optimization_score"], r["next_action"], r["silo"], r.get("page_visitors"), r.get("page_pageviews")) for r in kw["rows"]])
if mp:
    _vf = HERE / ".live/research/vercel/analytics.json"
    tr = (json.loads(_vf.read_text()).get("pages") if _vf.exists() else {}) or {}
    ...
    for silo, prs in mp["silos"].items():
        for r in prs:
            ...
            rows.append((run_id, r["url"], r.get("silo"), r.get("primary"), r.get("primary_volume"), ks.get("primary"), len(r.get("subs", [])), ks.get("subs_present"),
                         ...
                         (tr.get(r["url"].rstrip("/") or "/") or {}).get("visitors"), (tr.get(r["url"].rstrip("/") or "/") or {}).get("pageviews")))
    psycopg2.extras.execute_values(cur, "INSERT INTO seo_pages (run_id, url, silo, primary_keyword, primary_volume, primary_score, subs, subs_found, links_in_contextual, authority, gap_score, verdict, live_code, visitors_30d, pageviews_30d) VALUES %s ON CONFLICT DO NOTHING", rows)
